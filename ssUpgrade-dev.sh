...
if docker system prune --filter "until=24h" --force > /dev/null; then
    ...
    echo "No resources to prune or pruning failed"
fi

# Don't prune volumes by default to preserve important data
# If volumes need to be pruned, it should be an explicit operation
